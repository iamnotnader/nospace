...
public class OptimalSegmenter {
	...
	// This is a scoring function based on a first-order Makov model. If it can't find
	// a bigram probability, it returns the unigram probability instead. If it can't find
	// either a bigram probability or a unigram probability, it returns a smoothed value
	// minProb.
	...
	public static Node argmaxEnd(
		...
		for (int i = 0; i < nodes.size(); i++) {
			double currentG = g(nodes.get(i).str, "</s>", nodes.get(i).updatedGVal, unigrams, bigrams);
			if (currentG > maxG) {
				maxG = currentG;
				bestNodeIndex = i;
			}
	...
	// This function returns all perturbations of the input string. Right now we consider 
	// single and two-character perturbations.
	public static String[] adjacencies = {"qwsxz", "vfghn", "xsdfv", "swerfvcx", "wsdfr", "cdertgbv", "vfrtyhnb", "bgtyujmn", "ujklo", "yhnmkiu", "jmloiu", "kiop", "nhjk", "bghjm", "iklp", "ol", "asw", "edfgt", "zaqwedcx", "rfghy", "yhjki", "cdfgb", "qasde", "zasdc", "tghju", "xsa"};
		...
		for (int i = 0; i < s.length(); i++) {
			...
			for (int j = 0; j < adjacencies[charIndex].length(); j++) {
				if (Character.isLowerCase(currentChar)) {
					...
					//s.getChars(0, s.length(), tempString, 0);
					tempString[i] = Character.toUpperCase(adjacencies[charIndex].charAt(j));
				}

				Pair<String, Double> perturbation = new Pair<String, Double>(new String(tempString, 0, s.length()), errorProb);
				...
				for (int k1 = 0; k1 < adjacencies[index1].length(); k1++) {
					for (int k2 = 0; k2 < adjacencies[index2].length(); k2++) {
						...
						if (Character.isLowerCase(c1)) {
							...
							tempString[i] = Character.toUpperCase(adjacencies[index1].charAt(k1));
						}

						if (Character.isLetter(c2)) {
							tempString[j] = adjacencies[index2].charAt(k2);
						} else {
							tempString[j] = Character.toUpperCase(adjacencies[index2].charAt(k2));
						}

						Pair<String, Double> perturbation = new Pair<String, Double>(new String(tempString, 0, s.length()), 2*errorProb);
						ret.add(perturbation);
	...
	public static ArrayList<String> segmentGreedyBigram(
		...
		while (pos < input.length()) {
			...
			for (int i = pos + 1; i <= input.length(); i++) {
				double currentG = g(ret.get(ret.size() - 1), input.substring(pos, i), 0, unigrams, bigrams)/(i - pos);
				if (currentG > maxG) {
					maxG = currentG;
					bestPos = i;
				}
